...
//             responsibility. It displays groceries list, handles state
...
//                      should be enough to cover it. The behavior of whole
...
//                   Check out `test/05-Challenge-GroceryList.js` for tests to this part.
...
class GroceryList extends React.Component {
  ...
  inputChanged(event) {
    this.setState({ 
      ...
      disabled: (event.target.value.length > 0) ? false : true 
    });
  }
  ...
  toggleGroceryCompleteness(groceryIndex) {
    ...
    groceries[groceryIndex].completed = (groceries[groceryIndex].completed) ? '' : "completed";
  ...
  render() {
    ...
    newProductAddButton = <button className='add-product' onClick={this.addGroceryItem} disabled={this.state.disabled}>Add new Products</button>;
...
class GroceryListItem extends React.Component {
  ...
  render() {
    ...
    return (
      <li className={completed} onClick={this.props.onComplete} key={this.props.key}>
